      ...
      <div class="delete-content-wrapper">
        <div class="delete-card">
          ...
          <div class="target-input-box">
            <input type="text" value="{{ $menu->nama_menu ?? ($id ?? '') }}" readonly class="readonly-target-input">
          </div>

          <!-- ACTION BUTTONS -->
          <form action="{{ route('admin.menu.destroy', $menu->id ?? ($id ?? '')) }}" method="POST" class="delete-actions-form">
            @csrf
            @method('DELETE')
