...
<head>
  ...
  <title>Hapus Jam Operasional - Cafe Candaria</title>
  <link rel="stylesheet" href="{{ asset('css/hapusorganigram.css') }}">
    ...
    <main class="main-content">
      ...
      <div class="delete-content-wrapper">
        <div class="delete-card">
          ...
          <h2 class="delete-title">Hapus data jadwal ini?</h2>
          <p class="delete-description">
            Data jadwal jam operasional yang dihapus tidak bisa dikembalikan. Pastikan kamu yakin sebelum melanjutkan.
          </p>

          <!-- TARGET INPUT DISPLAY -->
          <div class="target-input-box">
            <input type="text" value="{{ $item->hari ?? ($id == 1 ? 'Senin - Jumat' : ($id == 2 ? 'Sabtu' : ($id == 3 ? 'Minggu' : 'Jadwal'))) }}" readonly class="readonly-target-input">
          </div>

          <!-- ACTION BUTTONS -->
          <form action="{{ route('admin.jam-operasional.destroy', $id ?? '') }}" method="POST" class="delete-actions-form">
            @csrf
            @method('DELETE')
